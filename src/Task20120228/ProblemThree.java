...
System.out.println("***************"+readid);
		  ...
		  continue;
				...
				for(int i=0;i<readsar.length;i++){
					if(cig[i] == 'D') continue;
					if(readsar[i] == 'C' && cig[i] == 'M'){
						 if(i+1<readsar.length && readsar[i+1]=='G' && cig[i+1]=='M'){
							 temprcg++;
						 }else{
							 temprncg++;
						 }
	...
	public ProblemThree() {
		...
		for(int i=1;i<=22;i++){
			readBwaMappedResults(bb+"Chr"+i+"_crick_CT");
		}
		readBwaMappedResults(bb+"ChrX_crick_CT");
		readMappedReads(base+"maxprecision/unique/crick_CT.txt",base2, base+"maxprecision/unique/watson_CT_mapping.txt",RefUtil.NEG);
		...
		readBwaMappedResults(bb+"ChrX_crick_CT");
		readMappedReads(base+"unmappble/bwa/FormatConvert/Fusion/maxprecision/filter/unique/crick_CT.txt", base2,base+"maxprecision/unique/watson_CT_mapping.txt",RefUtil.NEG);
