...
public class Consisitent {
		...
		try{
			...
			while(str != null){
				...
				for(int i=start;i<=end;i++){
					if( (i-start+1<seq.length()) && c[i-start]=='C' && c[i-start+1]==pp){ //只计算CG的深度
									cg[i]++; //计算CG位置深度
									total[i]++;
					}
					if( (i-start+1<seq.length()) && c[i-start]=='T' && c[i-start+1]==pp) //计算TG的深度
							       total[i]++;   //计算所有位置深度
				}
				str = br.readLine();
			}
	...
	public void readMappedReads_Crick(String filename,byte[] cg,byte[] total, long length){
		try{
			...
			while(str != null){
				...
				for(int i=start;i<=end;i++){
					if( (i-start+1<seq.length()) && c[i-start]=='C' && c[i-start+1]==pp){ //只计算CG的深度
								cg[(int) (length+1-i)]++; //计算CG位置深度
								total[(int) (length+1-i)]++;
					}
					if( (i-start+1<seq.length()) && c[i-start]=='T' && c[i-start+1]==pp) //只计算TG的深度
								total[(int) (length+1-i)]++;   //加上TG位置深度  
					}
				str = br.readLine();
			}
			br.close();
	...
	public Consisitent(String args[]) {
		if(args.length!=2){
			System.out.println("参数错误，请输入原始reads文件!以及CG/CA选项");
